...
import datetime
from inscricao.validators import email_validator, not_zero_validator, telefone_validator, escola_ano_validator, \
    smaller_zero_validator
...
class Atividade(models.Model):
    ...
    professor_universitario_utilizador_idutilizador = models.ForeignKey('ProfessorUniversitario', models.DO_NOTHING,
                                                                        db_column='professor_universitario_Utilizador_idutilizador')  # Field name made lowercase.
    unidade_organica_iduo = models.ForeignKey('UnidadeOrganica', models.DO_NOTHING,
                                              db_column='unidade_organica_idUO')  # Field name made lowercase.
    ...
    nrcolaborador = models.CharField(db_column='nrColaborador', max_length=45, blank=True,
...
class ColaboradorHasHorario(models.Model):
    ...
    horario_has_dia_id_dia_hora = models.ForeignKey('HorarioHasDia', models.DO_NOTHING,
                                                    db_column='horario_has_dia_id_dia_hora')
...
class HorarioHasDia(models.Model):
    ...
    def __str__(self):
        return 'No dia {} às {}'.format(self.dia_dia.dia, self.horario_hora.hora)
...
class Tarefa(models.Model):
    ...
    colaborador_utilizador_idutilizador = models.ForeignKey(Colaborador, models.DO_NOTHING,
                                                            db_column='colaborador_Utilizador_idutilizador', blank=True,
                                                            null=True)  # Field name made lowercase.
    ...
    buscar = models.ForeignKey(Espaco, models.DO_NOTHING, db_column='buscar', blank=True, null=True,
                               related_name='Tarefa_buscar')
    levar = models.ForeignKey(Espaco, models.DO_NOTHING, db_column='levar', blank=True, null=True,
                              related_name='Tarefa_levar')
    inscricao_coletiva_inscricao_idinscricao = models.ForeignKey(InscricaoColetiva, models.DO_NOTHING,
                                                                 db_column='inscricao_coletiva_inscricao_idinscricao',
...
class TransporteHasInscricao(models.Model):
    ...
    horario = models.ForeignKey(TransporteHasHorario, models.DO_NOTHING,
                                db_column='transporte_has_horario_id_transporte_has_horario')
    n_passageiros = models.IntegerField(validators=[smaller_zero_validator])

    def save(self, *args, **kwargs):
        TransporteHasHorario.objects.filter(id_transporte_has_horario=self.horario.pk).update(
            n_passageiros=F('n_passageiros') + self.n_passageiros)
        return super(TransporteHasInscricao, self).save(*args, **kwargs)

    def update(self, *args, **kwargs):
        old = TransporteHasInscricao.objects.filter(
            transporte_has_inscricao_id=self.transporte_has_inscricao_id).n_passageiros
        delta = self.n_passageiros - old
        TransporteHasHorario.objects.filter(id_transporte_has_horario=self.horario).update(
            n_passageiros=F('n_passageiros') + delta)
...
def delete_transporte(sender, instance, using, **kwargs):
    TransporteHasHorario.objects.filter(id_transporte_has_horario=instance.horario.pk).update(
        n_passageiros=F('n_passageiros') - instance.n_passageiros)
